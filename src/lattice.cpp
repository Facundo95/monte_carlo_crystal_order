...
void Lattice::loadInitialConfiguration(const std::string& filename) {
    ...
    if (ext == ".fl") {
        ...
        while (std::getline(redin, line)) {
            if (line.empty() || line.find("#") != std::string::npos || line.find("Lattice=") != std::string::npos || line.find("Properties=") != std::string::npos) continue;
            ...
            int specie = 0;
            if (tok == atom1) specie = 1;
            else if (tok == atom2) specie = 0;
            else if (tok == atom3) specie = -1;
            else {
 ...
 */
// `calculateDeltaChemicalEnergy` moved to `beg_hamiltonian.*`
